...
module.exports = function (Topics) {
	Topics.setBookmark = async function (tid, uid, index) {
		if (parseInt(uid, 10) <= 0) {
			return;
		}
		await db.sortedSetAdd(`uid:${uid}:bookmarks`, index, tid);
	};

	Topics.updateBookmark = async function (tid, pids) {
		...
		const bookmarks = await Topics.getBookmarks(tid);
		...
		await async.eachLimit(uidData, 50, async (data) => {
			...
			if (settings.postSort === 'most_votes') {
				return;
			}

			await Topics.setBookmark(tid, data.uid, bookmark);
		});
	};
